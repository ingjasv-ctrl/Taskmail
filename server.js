...
async function saveTokens(email, tokens) {
  ...
  try { await redisClient.set(`tokens:${email}`, JSON.stringify(tokens), { EX: 30*24*3600 }); } catch(e) {}
}
...
async function loadAllTokens() {
  ...
  try {
    ...
    for (const key of keys) {
      ...
      if (raw) { getAccount(email).tokens = JSON.parse(raw); console.log(`🔑 Tokens: ${email}`); }
    }
...
async function analyzeEmailWithClaude(email) {
  ...
  const prompt = `Analiza el siguiente correo y extrae TODAS las tareas o acciones requeridas.
...
async function syncAccount(email) {
  ...
  try {
    ...
    const messages = (listRes.data.messages || []).filter(m => !account.processedIds.has(m.id));
    if (messages.length === 0) { account.lastSync = new Date().toISOString(); return { success: true, newTasks: 0 }; }
    let totalNewTasks = 0;
...
cron.schedule("*/15 * * * *", () => {
  ...
  if (emails.length) emails.forEach(syncAccount);
});

// ══════════════════════════════════════════════════════════════
//  ARRANCAR — sesión PRIMERO, rutas DESPUÉS
// ══════════════════════════════════════════════════════════════
async function start() {
  ...
  // Sesión con Redis si está disponible
  let sessionConfig = {
  ...
  if (redisClient) {
    try {
      ...
      console.log("✅ Sesiones en Redis");
    } catch(e) {
      console.error("connect-redis error:", e.message);
    }
  }
  ...
  app.listen(PORT, () => {
    console.log(`\n🚀 TaskMail en http://localhost:${PORT}`);
    console.log(`💾 Redis: ${redisClient ? "✅" : "⚠️ memoria"}\n`);
  });
}
